...
const terminate = (server, options = { coredump: false, timeout: 500 }) => {
    ...
    return (code, reason) => (error, promise) => {
        console.log("SERVER_PROCESS_ERROR", `Process exited with code: ${String(code)}, reason: ${reason}`);
        if (error && error instanceof Error) console.log("SERVER_PROCESS_ERROR", `Uncaught Exception: ${String(error.message)}`);
